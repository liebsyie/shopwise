...
export function canvasMode(opts = {}) {
    ...
    return {
        ...
        resolveId(id) {
            switch (id) {
                case 'virtual:canvas-mode-injected':
                case 'virtual:canvas-mode-constants':
                    return `\0` + id;
            }
        },
        // Inject whatever client behavior we want in the child document to support canvas mode.
        transformIndexHtml(html) {
            return {
                ...
                tags: [
                    {
                        tag: 'script',
                        attrs: { type: 'module', src: '/@id/virtual:canvas-mode-injected' },
                        injectTo: 'body',
                    },
                ],
            };
        ...
        load(id) {
            ...
            switch (id) {
                case `\0virtual:canvas-mode-injected`:
                    const filePath = path.resolve(__dirname, 'injected.js');
                    return fs.readFileSync(filePath, 'utf-8');
                case `\0virtual:canvas-mode-constants`:
        ...
        configureServer(server) {
            server.ws.on('mocha:canvas:toggle-canvas-mode', async ({ isEnabled }) => {
                ...
                for (const mod of server.moduleGraph.idToModuleMap.values()) {
                    ...
                    if (!mod.file.endsWith('.tsx') && !mod.file.endsWith('.jsx'))
                        continue;
                    invalidateModule(mod.file.replace('/app', ''), server);
                }
            });
            server.ws.on('mocha:canvas:update-element', async ({ elementId, text }, client) => {
                try {
                    ...
                    invalidateModule(filePath, server);
...
async function invalidateModule(filePath, server) {
